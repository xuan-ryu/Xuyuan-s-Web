...
const PLATES = [
  {
    ...
    width: 1246,
    height: 1759,
    alt: "Character concept sheet for Wan Shouren: full-figure concept art, four expression studies, turnaround views, pixel-art sprites, and archival photographs and film-still references.",
  },
  {
...
export function HungerPosterLayout({ project }: { project: Project }) {
  ...
  // Each document ships a pre-compressed JPEG sibling for the loupe background
  // (`<name>-loupe.jpg`, generated by sharp) so hovering never re-downloads
  // the full PNG the quiet CTAs link to.
  const loupeFor = (src: string) => src.replace(/\.png$/, "-loupe.jpg");
    ...
    <article className="poster-page hunger-page">
      ...
      <section className="hunger-sheet" aria-label="The printed broadsheet">
        <div className="hunger-sheet-media" data-fade>
          <HungerLoupeFrame
            src={poster.image}
            loupeSrc={loupeFor(poster.image)}
            alt={SHEET_ALT}
            width={1243}
            height={1762}
      ...
      <section className="hunger-section hunger-dispatch" aria-labelledby="hunger-dispatch-title">
        ...
        {/* The claim carries the page's one un-restated design fact — the
            moral-value system (printed on the broadsheet and Plate 01): each
            survival choice is weighed, and the weight branches the ending. */}
        <h2 id="hunger-dispatch-title" className="hunger-claim" data-fade>
          Survive how you choose — a moral-value system weighs every decision
          and branches the ending.
        </h2>
      ...
      <section className="hunger-section hunger-motion" aria-labelledby="hunger-motion-title">
        ...
        <h2 id="hunger-motion-title" className="hunger-claim hunger-motion-title" data-fade>
        ...
        {project.previewVideo && (
          <div className="hunger-reel" data-fade>
            <OffscreenVideo
              ...
              poster="/media/work/hunger/preview-poster.jpg"
              threshold={0.5}
            ...
            {plates.map((plate, i) => (
              <li className="hunger-plate" key={plate.src}>
                <div className="hunger-plate-caption">
                  ...
                  {/* one cue for the whole archive — same form as the
                      broadsheet's, shown only where the loupe exists */}
                  {i === 0 && (
                    <p className="hunger-plate-cue">
                      <InteractiveCue accent="var(--case-accent)">
                        Hover a plate — the loupe magnifies
                      </InteractiveCue>
                    </p>
                  )}
                </div>
...
/* loupe cues — name the gesture; only shown where the loupe exists
   (hover-capable fine pointers, same gate as .hunger-loupe itself).
   The broadsheet cue sits on its own row under the caption column so it
   never crowds the caption/CTA line; the archive repeats the same form
   once, in Plate 01's caption column. */
.hunger-loupe-cue {
  display: none;
  grid-column: 1 / 7;
  grid-row: 3;
...
/* h2 peer of the other section claims — .hunger-claim carries the type;
   this class only places it and restores the pre-media margin */
.hunger-motion-title {
  grid-column: 1 / 7;
  grid-row: 3;
...
@media (max-width: 1079px) {
  ...
  /* the page's one seal-red survives below 1080: the chop shrinks and
     signs off the end of the Dispatch column instead of vanishing */
  .hunger-stamp {
    grid-column: 3 / -1;
    grid-row: 5;
    justify-self: start;
